      ...
      setdate(new Date().toLocaleDateString());
        const fetchBlogData = async()=>{
          try{
            ...
            if(data){
              setform({slug: data.slug||"", title: data.title||"", subtitle: data.subtitle||"", content: data.content||"", hero_image: data.hero_image||""});
            }}
            catch(err){
              console.log("Couldn't fetch blog data ", err)
            }
          };
          fetchBlogData();
    ...
    const handleSubmit = async (e) => {
      ...
      const url = "/api/blogs"
        ...
        if(response?.ok){
          ...
          toast.error(`Blog couldn't be saved`, {
            position: "top-right",
            autoClose: 2000,
            hideProgressBar: false,
            closeOnClick: false,
    ...
    <div className='w-full'>
      <form className=' mx-10 my-5'>
        <div className="mb-4">
          <div className='flex gap-1'>
            ...
            <p className='text-red-500 flex items-center justify-center text-sm '>*</p>
          </div>
        ...
        <div className="mb-4">
            <label htmlFor="Subtitle" className=" mb-2 text-sm  flex font-medium text-gray-800">Subtitle <p className='mx-1 flex items-center justify-center text-xs text-slate-700'>(Optional)</p></label>
        ...
        <div className='mb-4'>
        <div className='flex gap-1'>
            <label htmlFor="content" className="block mb-2 text-sm  text-gray-800">Content</label>
            <p className='text-red-500 flex items-center justify-center text-sm '>*</p>
          </div>
        <TiptapEditor 
        value={form.content}
      ...
      <div className='flex gap-1'>
        ...
        <div className="buttons flex gap-2 items-center mr-10 mb-10 justify-end">
            ...
            className="text-white mt-2 mr-3 cursor-pointer bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-1.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">Submit</button>
